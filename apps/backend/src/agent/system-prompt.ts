...
export function buildSystemPrompt(input: {
  ...
  if (input.memory) {
    memoryLines.push(
      "",
      "## 长期记忆",
      "你拥有跨会话持久的长期记忆，存放在 /memories 目录（独立于工作目录），通过 memory 工具读写。",
  ...
  return [
    ...
    "- shell 命令默认最多前台等待 15 秒；如果预计命令会长时间运行、持续监听或没有明确结束点，调用 shell 时设置 background=true 让它立即转入后台。后台命令的完整输出会持续写入返回的文件路径，后续用 read_file 分段查看输出，用 shell_status 查看是否结束；如果命令没有进展、卡住或不再需要，用 shell_cancel 终止它。",
