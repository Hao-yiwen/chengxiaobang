...
function buildSummary(text: string, relativePath: string, context: ToolResultSpillContext): string {
  ...
  return [
    ...
    "",
    "你可以按需分段查看：",
    `- 读取开头：调用 read_file，参数为 {"path":"${relativePath}","startLine":1,"lineLimit":120}`,
    `- 读取指定区间：调用 read_file，参数为 {"path":"${relativePath}","startLine":121,"lineLimit":120}，并按需要调整 startLine`,
    `- 搜索关键词：调用 search，参数为 {"path":"${TOOL_RESULT_SPILL_DIR}","query":"关键词"}`,
    "",
    "结果开头预览：",
    head,
    "",
