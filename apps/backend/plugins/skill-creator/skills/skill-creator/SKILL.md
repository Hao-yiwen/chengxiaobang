...
A skill for authoring and iteratively improving local skills.
...
The app discovers skills in these directories (highest priority first):

- `<project>/.chengxiaobang/skills/<name>/SKILL.md`
- `~/.chengxiaobang/skills/<name>/SKILL.md`

Use the `<project>` path for skills that only make sense in the current repo. Use the `~/` path for personal skills you want available everywhere.
...
Optional reserved frontmatter fields may be supported later; for most skills you only need `name` and `description`.

### Progressive disclosure

The app loads skills in three layers:
...
Then run them: load the draft skill, hand the model the test prompt, and inspect what happens. The app does not currently spawn parallel evaluation subagents, so do this one prompt at a time and look at each result with the user.
...
1. Make sure the draft skill is on disk where the app can discover it (one of the directories listed above).
2. In a fresh turn, give the test prompt to the model. Either let the description trigger the skill, or use `/skill <name> <prompt>` to force-load it.
