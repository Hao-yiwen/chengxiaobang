...
export function withCurrentComposerModel(
  ...
  const fallback = mergeProviderConfigModelOptions(provider, provider.models ?? [], provider.model);
  ...
  return [...options, resolveProviderConfigModelOption(provider, currentModel)];
}
