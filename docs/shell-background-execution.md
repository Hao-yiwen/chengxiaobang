...
模型经常会通过 `Bash`（macOS/Linux/Windows 可见）或 `PowerShell`（仅 Windows 可见）工具运行测试、构建、安装依赖或调试脚本。这类命令有两个问题：
...
- 默认不传 `timeout`：前台最多等待 **15 秒**；超过后返回后台命令信息，模型继续下一步决策。
...
`Bash`、Windows-only `PowerShell`、`GitStatus`、`GitDiff` 都通过同一条 shell 工具执行链路运行：

```text
模型调用 Bash / PowerShell
        │
        ▼
...
`timeout` / `run_in_background` 只决定本次工具调用等待多久，不决定命令是否被强制终止。除了用户中止 run 或模型主动调用 `BashCancel`，慢命令在等待窗口结束后会继续作为后台命令运行。后台查询与取消统一使用 `BashStatus` / `BashCancel`，即使原始命令来自 `PowerShell` 也是如此。
...
- 用 `Read` 读取输出文件。
- 用 `BashStatus` 查看命令是否结束。
- 如果命令没有进展、卡住或不再需要，用 `BashCancel` 终止。
...
- 默认：15 秒内结束则直接返回命令输出；超过 15 秒返回后台命令 ID、PID、输出文件路径和后续操作提示。
- `run_in_background=true`：启动后立即返回后台命令 ID、PID、输出文件路径和后续操作提示。
- `timeout`：在指定等待窗口内结束则直接返回命令输出；超过 `timeout` 返回后台命令信息，命令继续执行。
- 前台等待期间收到 run abort：终止进程组并返回中止结果。

### `PowerShell`

仅在 `process.platform === "win32"` 时注册。在当前工作目录用 Windows 原生 PowerShell 执行命令，参数与 `Bash` 相同：
...
行为与 `Bash` 共享同一套等待、后台转入、输出落盘、run abort 和审批语义。它适合 `Get-ChildItem`、`Select-String`、`Test-Path` 等 PowerShell 语法；后台命令仍用 `BashStatus` 查询、用 `BashCancel` 终止。

### `GitStatus` / `GitDiff`

这两个工具仍然是 read-only 工具，但底层同样走 shell 执行链路：

- `GitStatus` 执行 `git status --short --branch`。
- `GitDiff` 执行 `git diff --stat && git diff --check`。
...
`BashStatus` 只查询当前 backend 进程内记录的后台命令；如果 backend 重启，旧命令记录会丢失。
...
它只作用于 `Bash` 工具返回的后台命令 ID，不接受任意 PID。终止时会先发 `SIGTERM`，若短时间内没有退出，再升级为 `SIGKILL`。

如果命令已经结束，`BashCancel` 会返回当前快照，不会重复杀进程。
...
- **后台阶段**：命令已经从本次工具调用释放，run 不再继续等待它；此时需要模型或用户通过 `BashCancel` 主动终止。
...
| `aborted` | 命令被 run abort 或 `BashCancel` 终止 |

后台命令结束后，记录仍保留在当前 backend 进程内，模型可以继续用 `BashStatus` 查询最终状态并读取输出文件。
...
- backend 重启后，旧输出文件仍在工作区里，但旧后台命令无法再通过 `BashStatus` 查询，也无法通过 `BashCancel` 终止。
- 输出文件不做自动清理；后续如需要，可以单独设计工作区缓存清理策略。
- `BashStatus` / `BashCancel` 是 read-only 审批分类，因为它们只能作用于应用自己创建的后台命令 ID，不能直接执行任意 shell 命令。
...
- `apps/backend/src/tools/shell-tools.ts`：pi 工具封装、`timeout` / `run_in_background` 参数归一化、Windows `PowerShell` 注册、后台提示文案、`BashStatus` / `BashCancel`。
- `packages/shared/src/tool.ts`：内置工具名契约。
- `Bash` / `PowerShell` 工具描述与后台工具返回结果：提示模型如何读取后台输出、查询状态和取消慢命令。
