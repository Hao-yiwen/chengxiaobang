...
本文说明程小帮的 shell 工具异步执行链路。目标是避免 `shell` 工具被长时间运行的命令卡住：短命令仍然直接返回结果；超过前台等待阈值的命令会转入后台继续执行；模型也可以通过 `background: true` 明确要求命令立即后台运行。完整输出落到工作区文件里，由模型按需读取、查询状态或主动终止。
...
这两个工具仍然是 read-only 工具，但底层同样走 shell 异步执行链路：
