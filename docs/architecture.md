...
│       │                     #   direct-commands、workspace、slash-command-service、
...
| `ProviderConfig` | `kind`(deepseek/kimi/minimax/…/custom)、`baseURL`、`model`、`apiKeyRef`(密钥引用,非明文) |
...
| `/api/settings/*` | provider 管理与连通性测试、飞书配置 |
...
1. **解析 provider 与 API key**(SecretStore),无可用模型直接抛错(路由层转成 `run_end(failed)` 的 SSE)。
...
6. **直接斜杠命令快速路径**(`/ls /read /write /shell /git status /git diff`,`tools/direct-commands.ts`):不经模型直接执行单个内置工具,同样走审批门控;结果落为无 payload 的 tool 行;失败则 `run_end(failed)` 结束,成功则**继续**进入模型循环让模型基于结果作答。
7. **pi 循环**:`runPiLoop` 用 `agent/history.ts` 从持久化行重建 pi 对话,调 pi 的 `runAgentLoopContinue(context, config, emit, signal, streamFn)`。
...
```ts
{
  ...
  shouldStopAfterTurn: translator.shouldStopAfterTurn  // 中止 + 25 轮上限
}
```
...
| `agent_end` | **唯一终态决策点**:中止→aborted;错误→failed;25 轮上限→failed(中文报错);否则 completed+usage |
...
- **读**(`agent/history.ts` `buildAgentMessages`):有 payload 的行直接反序列化为 pi 消息——assistant 的 toolCall 块与 toolResult 消息**无损配对回放**,模型在后续 run 中看到完整的工具调用历史;无 payload 的行(旧数据、直接斜杠命令结果)回退为纯文本消息(tool 行折叠为 `【工具结果】` user 上下文,因为孤儿 toolResult 会被 provider 拒收)。
...
- `model/pi-model.ts`:`buildModel(provider)` 把 `ProviderConfig` 映射为 pi-ai `Model<"openai-completions">`。内置 kind 映射 pi slug(deepseek→`deepseek`、kimi→`moonshotai`、minimax→`minimax`),其余 kind 原样传递——pi 按 **slug 或 baseUrl** 自动探测线上兼容差异(DeepSeek 的 `reasoning_content` 与 `requiresReasoningContentOnAssistantMessages`、Moonshot 的 `max_tokens` 字段等),无需手写 compat。`reasoning: false` 表示不发 thinking 请求参数,但 provider 自己返回的 reasoning 增量照常透出。
...
防呆边界:迭代上限 25 轮(超出 → `run_end(failed)` 中文报错);SSE 15s 心跳 + Bun idleTimeout 255s 保审批长等待;`agent_end` 兜底保证 run 永有终态。
...
- 关键行为钉死在:黄金事件序列(全事件顺序)、审批通过/拒绝、流中/审批中中止、25 轮上限、payload 无损回放与孤儿修复、DeepSeek wire 测试(stub fetch 喂真实 SSE 字节,走真实 pi-ai 解析)、SSE 端到端(经 `createApp` + `parseSseChunk`)。
...
| 直接斜杠命令保留 | 确定性单工具语义是产品功能;执行后仍进模型循环让模型基于结果作答 |
